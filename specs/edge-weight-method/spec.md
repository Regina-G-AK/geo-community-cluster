...
- `transaction_count` 模式的实验统计 MUST 将 SPPMI 阶段标记为已跳过，并沿用最小支持人数过滤后的商户数和边数。
- 自动分析损失最大阶段时，已跳过的 SPPMI 阶段 MUST 按零损失处理且不参与最大损失阶段竞争。
...
- 实验记录 MUST 仅在结果成功落盘后追加，并包含实际运行目录、边权重计算方式、完整参数快照、阶段统计和现有规范要求的分析指标。
- 参数实验 MUST 使用新的自动命名运行目录并保留历史结果。
- README、自动实验记录逻辑和测试 MUST 随实现同步更新。
...
- 阶段统计显式记录 SPPMI 阶段是否执行，使跳过状态与零损失语义可被实验记录和自动分析复用。
